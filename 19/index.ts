function learn(time: number, courses: number[]) {
  ...
  let finalCourses: number[] = [];

  function getMaxNumber() {
    let maxNumber = Math.max(...coursesArr);
    let indexMaxNumber = coursesArr.indexOf(maxNumber);
    coursesArr.splice(indexMaxNumber, 1);

    if (finalCourses.includes(time) || maxNumber === time) {
      return getMaxNumber();
    } else if (maxNumber > time) {
      return null;
    } else {
      finalCourses.push(maxNumber);
      let final = finalCourses[0] + finalCourses[1];
      if (final > time) {
        finalCourses.pop();
        getMaxNumber();
      }
  ...
  return finalCourses.length === 0 ? null : [courses.indexOf(finalCourses[0]), courses.indexOf(finalCourses[1])];
}
